...
CREATE TABLE IF NOT EXISTS `dl_leika` (
  ...
  `leika_nr` varchar(500) DEFAULT NULL,
  `leika_bezeichnung` varchar(500) DEFAULT NULL,
  `leika_bezeichnung2` longtext,
...
CREATE TABLE IF NOT EXISTS `dl_ozg` (
  ...
  `lage` int DEFAULT NULL,
  `ozg_nr` varchar(500) DEFAULT NULL,
  `ozg_leistung` longtext,
  `ozg_zuständigkeit` varchar(500) DEFAULT NULL,
  `ozg_fachlichkeit` varchar(500) DEFAULT NULL,
  `ozg_regelungskompetenz` varchar(500) DEFAULT NULL,
  `ozg_regelungskompetenz_ministerium` varchar(500) DEFAULT NULL,
  `ozg_leika_leistungen` longtext,
  PRIMARY KEY (`ozg_id`),
  KEY `FK_dl_ozg_dl_ozg_kategorien` (`kategorie`),
  KEY `FK_dl_ozg_dl_ozg_lagen` (`lage`),
  CONSTRAINT `FK_dl_ozg_dl_ozg_kategorien` FOREIGN KEY (`kategorie`) REFERENCES `dl_ozg_kategorien` (`ozgkategorie_id`),
  CONSTRAINT `FK_dl_ozg_dl_ozg_lagen` FOREIGN KEY (`lage`) REFERENCES `dl_ozg_lagen` (`lage_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
-- Exportiere Struktur von Tabelle serviceportal.dl_ozg_lagen
CREATE TABLE IF NOT EXISTS `dl_ozg_lagen` (
  `lage_id` int NOT NULL AUTO_INCREMENT,
  `lage` varchar(500) DEFAULT NULL,
  PRIMARY KEY (`lage_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;

-- Exportiere Daten aus Tabelle serviceportal.dl_ozg_lagen: ~0 rows (ungefähr)
/*!40000 ALTER TABLE `dl_ozg_lagen` DISABLE KEYS */;
/*!40000 ALTER TABLE `dl_ozg_lagen` ENABLE KEYS */;
